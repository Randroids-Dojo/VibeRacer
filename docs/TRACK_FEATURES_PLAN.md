...
**0a. Per-piece width resolution.** New module `src/game/trackWidth.ts` exporting `DEFAULT_TRACK_WIDTH = 8`, `widthAt(op, t)`, `halfWidthAt(op, t)`. Default impl returns `DEFAULT_TRACK_WIDTH` for every piece. Replace the 14 direct `TRACK_WIDTH` references in `wheelContact.ts:104`, `kerbs.ts:76,128`, `scenery.ts:38,66,335,373`, `sceneBuilder.ts:279,299,300,332,1201,1202,2158,2176`, `minimap.ts:101`, `lib/trackThumbnail.ts:21` with `widthAt` / `halfWidthAt` calls. Re-export `TRACK_WIDTH` from `trackPath.ts` so existing imports compile unchanged.
