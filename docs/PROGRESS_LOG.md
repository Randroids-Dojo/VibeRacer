...
- Verification: dash checks (no U+2014 or U+2013 anywhere in src / tests / docs), `git diff --check`, `npm run type-check`, `npm test` passed with 3256 tests (two new continuous-angle closure tests plus five residual-snap regression tests), focused `npx playwright test tests/e2e/smoke.spec.ts --grep "track editor"` passed with 9 tests, and `npm run build` passed.
