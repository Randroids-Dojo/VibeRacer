...
- Files: new `src/lib/continuousTuning.ts` (`suggestContinuousTuningTweaks`, `applyContinuousSuggestion`, `CONTINUOUS_TUNING_MAX_SUGGESTIONS`, plus eight scored heuristics: low-speed off-track split by entry-steer magnitude into "turn faster" vs "calmer steering" picks, low-speed path-wiggle detection from the position trace, high-speed off-track, never-reached top speed, always-at top speed, long off-track durations, frequent excursions, near-stopped crawl share). The assembly step resolves contradictory pairs (sharpen vs soften the same param) so the freeze panel never shows both directions in the same lap. Updated `src/components/TuningSession.tsx` to add a `continuous` phase, route lap-complete into it, render a `ContinuousSuggestView` overlay above the paused canvas, and relabel the drive HUD's Stop run button to End session. Updated `src/components/TuningLab.tsx` and `IntroView`'s blurbs to describe the new loop. Updated `docs/GDD.md`'s Interactive Tuning Lab section to document the continuous flow + heuristics module.
- Tests: new `tests/unit/continuousTuning.test.ts` (16 tests covering empty-input no-ops, each heuristic firing on its trigger telemetry, the low-speed full-lock vs modest-steer split, synthetic swervy-path detection and steady-corner non-detection, the high-speed top-cap suppression when high-speed off-tracks are present, score-sorting + max-result truncation, clamped no-op filtering, no-telemetry fallback, and the apply helper's add + clamp behavior).
