...
## 2026-05-24, Tuning Lab continuous-lap loop

- Branch: `claude/continuous-tuning-lap-bIODR`
- Changed: rewired the Tuning Lab session flow so it no longer stops after one lap to ask for a Likert survey. After every completed lap the rAF loop freezes, runs a new heuristics module against the lap's off-track events + per-sample speed trace, and offers the player up to three small `CarParams` tweaks (each labeled, reasoned, and shown with a delta chip) plus a "No change, drive again" CTA. Picking an option applies the delta through the clamp-aware `applyContinuousSuggestion`, records it in tuning history with `source: 'recommended'`, and re-enters countdown for the next lap. The classic 5-point Likert form + recommendation diff + save flow is preserved and runs on the player's "End session and review" exit (also on the in-drive End session button, which used to be labeled Stop run) so the saved-tuning library still gets named entries.
- Files: new `src/lib/continuousTuning.ts` (`suggestContinuousTuningTweaks`, `applyContinuousSuggestion`, `CONTINUOUS_TUNING_MAX_SUGGESTIONS`, plus nine scored heuristics: low-speed off-track split by entry-steer magnitude into "turn faster" vs "calmer steering" picks, low-speed path-wiggle detection from the position trace, high-speed off-track split the same way, high-speed path-wiggle that fires on straight-line correction oscillation without any off-track event, never-reached top speed, always-at top speed, long off-track durations, frequent excursions, near-stopped crawl share). The assembly step resolves contradictory pairs (sharpen vs soften the same param, low or high speed) so the freeze panel never shows both directions in the same lap. Updated `src/components/TuningSession.tsx` to add a `continuous` phase, route lap-complete into it, render a `ContinuousSuggestView` overlay above the paused canvas, relabel the drive HUD's Stop run button to End session, embed a collapsible "Show all tuning sliders" region inside the freeze panel, AND add a Tuning button to the in-drive HUD that pauses the lap clock and opens the same live-apply `TuningEditor` mid-drive (the lap clock compensates for the paused interval through `resumeShiftRef`, mirroring the `Game.tsx` pause / resume pattern). Every manual slider tweak applies straight to the live car for the next lap and records as `source: 'slider'` in tuning history. Per AGENTS.md RULE 3.5, the New tuning session intro screen is now rendered through `MenuPageShell` with `MenuPickRow` rows for the control + track-tag chips, `MenuStartButton` for the Start drive CTA, and `MenuShellAction` for Back, so the intro reads as part of the same family as Free Race / Derby / Tour / Settings / PreRaceSetup instead of the previous dev-console dark card. `TuningLab.tsx`'s `sessionShellStyle` lost its dark gradient and now paints `menuTheme.pageBg` so the menu family bleeds through every TuningSession phase that doesn't paint its own surface. The drive HUD chrome (chip + action row) is now driven by an injected CSS class block (`DRIVE_HUD_CSS`) instead of inline `CSSProperties`, mirroring the idiomatic pattern used by `Game.tsx`'s `PAUSE_BUTTON_CSS`: a `position: fixed; inset: 0` wrap holds an `absolute` chip and action row, both with `env(safe-area-inset-*)` on the relevant top / right / bottom / left edges. A `@media (any-pointer: coarse)` block bumps the top / right / bottom insets, font sizes, padding, and the minimum touch target (44 px desktop / 48 px coarse) so the chip and the Restart / Tuning / End session row stop kissing the screen edge on Android / iOS phones. Every edge inset uses `max(<floor>, calc(<base> + env(safe-area-inset-*)))` so a phone that reports zero safe-area envs (Android Chrome on a non-notched display, where the URL bar and gesture indicator are not exposed via env) still gets a generous visual gap (20 / 32 px desktop, 28 / 48 px coarse). A `@media (max-width: 380px)` block flexes the action row across the full row width so narrow phones never push End session into the right margin. Added a top-level `viewport` export in `src/app/layout.tsx` with `viewportFit: 'cover'` so `env(safe-area-inset-*)` actually returns non-zero values on devices with display cutouts. Updated `src/components/TuningLab.tsx` and `IntroView`'s blurbs to describe the new loop. Updated `docs/GDD.md`'s Interactive Tuning Lab section to document the continuous flow + heuristics module.
- Tests: new `tests/unit/continuousTuning.test.ts` (19 tests covering empty-input no-ops, each heuristic firing on its trigger telemetry, the low- and high-speed full-lock vs modest-steer splits, synthetic swervy-path detection at both speed bands, steady-corner and clean-straight non-detection, the high-speed top-cap suppression when high-speed off-tracks are present, score-sorting + max-result truncation, clamped no-op filtering, no-telemetry fallback, and the apply helper's add + clamp behavior).
- Verification: `npx vitest run tests/unit/continuousTuning.test.ts tests/unit/tuningLab.test.ts tests/unit/tuningSettings.test.ts` (72 passed), `npm run type-check` (clean), `npm run lint` (no new warnings, only the pre-existing RaceCanvas / TrackEditor / useGamepad ones).
- Followups: a Playwright smoke that drives two laps in the lab and asserts the suggestion overlay shows up between them would catch a future regression in the phase machine; deferred so the slice stays focused on the loop logic + heuristics. The heuristics module also accepts an unused `acceptedHistory` slot in spirit (we could feed it the chosen suggestion ids across laps to soft-suppress repeats), but the v1 set already debounces itself by removing clamped no-ops.
...
- Changed: verified every open PR was merged into `main` (`#126`, `#134`, `#139`, `#140`) and reconciled docs with the final merged state. Drag mode now has the gauge speedometer, selected ghost-source presentation tags, and Playwright coverage for visible needle movement. Derby now combines hard physical collision impulse with a 15-point per-hit damage cap, 350 ms per-pair damage cooldown, pushable wrecks, wreck friction, and current authored GLB assets.
- Verification: `gh pr list --state open` returned no open PRs. Checked current `origin/main` docs for stale PR references and updated `docs/PROGRESS_LOG.md`, `docs/GDD.md`, `docs/GDD_COVERAGE.json`, and `docs/FOLLOWUPS.md` to match the merged code.
- GDD coverage: added the missing Section 19 Drag Racing coverage row and corrected Section 20 Derby coverage gaps.

## 2026-05-14, Derby Impact Pass

- Branch: `fix/derby-impact-docs-current`
- Changed: made derby car-car collisions hit harder through a stronger impact scale and a mass-aware velocity impulse after OBB contact so the attacker slows and the victim gets shoved away instead of only taking health damage. The later wreck-physics merge keeps that hard physical hit feel but restores the per-hit damage cap to 15 for slower TTK, with a 350 ms per-pair damage cooldown. Also carried the model-viewer review cleanup from the merged GLB branch: transparent shared renderer tiles, cached renderer resizing, toggle state, model-tile animation coverage, the Blender bisect cap fix, and current Derby vehicle labels.
...
- GDD coverage: Section 20 and `docs/GDD_COVERAGE.json` record the hard physical impact tuning, capped damage, collision impulse behavior, current displayed vehicle names, and shipped GLB loader status.
