...
6. **Text selection is off by default.** The app is a game, not a document. `src/app/globals.css` disables `user-select` and `-webkit-touch-callout` on `html, body` and re-enables them only on `input`, `textarea`, `select`, and `[contenteditable]`. Do not add `user-select: text` to a menu, overlay, HUD, leaderboard, or stat tile. Players cannot copy slug names or lap times on purpose. If you need a typeable field, use a real form input and it will work automatically.
...
- **Text selection default.** The app-wide non-selectable default is enforced in `src/app/globals.css`. Do not re-enable selection on game or menu surfaces. Form inputs (`input`, `textarea`, `select`, `[contenteditable]`) opt back in via the same stylesheet, so use real form elements and they work. The `userSelect: 'none'` lines inside `MOBILE_GAME_SURFACE_STYLES` are intentionally redundant defense in depth and should stay.
